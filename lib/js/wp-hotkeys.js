...
/* To do
	- add up down functionality when inside a sub menu that's already open since it's part of the current page
	- choose better defaults and format
*/
...
function wphSetupHotkey(hotkey, url, item, menuLink, subItems) {
	...
	if ( hotkey ) {
		...
		jQuery.hotkeys.add(hotkey, {type:type, propogate:propogate}, function() {
...
function wphManualResets() {
	...
	jQuery('html').not(topLevelLinks).click(function( event ) {
		wphReset();
	});
}
