...
jQuery(document).ready(function() {
	...
	// Setup functionality to close sub-menus with 1. escape key or 2. mouseout
	if (closeHoverHotkey) {
		jQuery.hotkeys.add(closeHoverHotkey, {type:'keydown'}, function() {
	    	whReset();
	    });
	}

	/*
	jQuery(topLevelLinks).parents('li').mouseout(function() {
		whReset();
	});
...
function whHotkeys() {
	...
	jQuery.each(menuItems, function() {
		...
		if (url && hotkey) {
		    ...
		    jQuery.hotkeys.add(hotkey, {type:'keydown', propagate: true}, function() {
...
function whSetupHotkey(hotkey, url) {
	...
	if (hotkey) {
	    ...
	    jQuery.hotkeys.add(hotkey, {type:'keydown'}, function() {
